# Save this as: download-service.ps1
$url = "https://pastebin.com/raw/PLACEHOLDER"  # I'll provide correct content below
$output = "F:\NewKodiProjects\myIPTV\script.iptv.autoconfig\service.py"

# Backup old file
if (Test-Path $output) {
    Copy-Item $output "$output.backup"
}

# Create the complete service.py content directly
$content = @'
import xbmc
import xbmcaddon
import xbmcgui
...
def enable_addon(addon_id):
    try:
        ...
        json_query = json.dumps({'jsonrpc': '2.0','method': 'Addons.SetAddonEnabled','params': {'addonid': addon_id, 'enabled': True},'id': 1})
...
def set_skin(skin_id):
    try:
        log('Setting skin to: %s' % skin_id)
        json_query = json.dumps({'jsonrpc': '2.0','method': 'Settings.SetSettingValue','params': {'setting': 'lookandfeel.skin', 'value': skin_id},'id': 1})
        result = xbmc.executeJSONRPC(json_query)
        log('Set skin JSON-RPC result: %s' % result)
        xbmc.executebuiltin('ReloadSkin()')
        xbmc.sleep(3000)
        log('Skin change initiated')
        return True
    except Exception as e:
        log('Failed to set skin: %s' % str(e))
        return False
...
def hide_menu_items():
    try:
        ...
        log('Arctic Zephyr menu configured')
        return True
    except Exception as e:
...
def main():
    ...
    if not dialog.yesno(ADDON_NAME, 'Auto-configure IPTV for %s?\n\nThis will:\n- Install PVR IPTV Simple\n- Download 3000+ channels\n- Install Arctic Zephyr skin\n- Configure clean menu' % country_name):
        return
    progress = xbmcgui.DialogProgress()
    progress.create(ADDON_NAME, 'Setting up IPTV...')
    progress.update(10, 'Checking PVR...')
    pvr_path = os.path.join(ADDONS_PATH, 'pvr.iptvsimple')
    if os.path.exists(pvr_path):
        log('PVR already installed')
        pvr_installed = True
    else:
    ...
    if not pvr_installed:
        progress.close()
        dialog.ok(ADDON_NAME, 'Failed to install PVR')
        return
...
Write-Host "✅ service.py created successfully!" -ForegroundColor Green
